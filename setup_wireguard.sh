...
input_full_config() {
    ...
    echo -e "${YELLOW}Вставьте полный конфиг WireGuard (нажмите Ctrl+D после вставки):${NC}"
    ...
    # Очистка конфига от служебных строк и пустых строк
    config_content=$(grep -v "^[=#].*===\|^Вставьте\|^$" "$temp_file" | sed '/^$/N;/^\n$/d')
...
create_config() {
    read -p "Введите имя интерфейса (например, wg0): " interface_name
...
main() {
    ...
    case $main_choice in
        1)
            ...
            ;;
        2)
            read -p "Введите имя интерфейса для управления: " interface_name
